# REGWQ -------------------------------------
regwq <- function (y, trt, DFerror, MSerror, alpha = 0.05, group = TRUE, 
                   main = NULL, console = FALSE) 
{
  ...
  Tprob <- NULL
  if (ntr > 3) {
    for (i in 2:(ntr - 2)) Tprob[i - 1] <- qtukey(p = (1 - alpha)^(i/ntr), 
                                                  i, df = DFerror)
  }
  Tprob[ntr - 2] <- qtukey(p = (1 - alpha), ntr - 1, df = DFerror)
  Tprob[ntr - 1] <- qtukey(p = (1 - alpha), ntr, df = DFerror)
  if (Tprob[ntr - 2] > Tprob[ntr - 1]) 
    Tprob[ntr - 1] <- Tprob[ntr - 2]
  nr <- unique(nn[, 2])
  if (console) {
    cat("\nStudy:", main)
  ...
  for (k in 1:nn) {
    ...
    if (odif[k] <= ntr - 2) 
      pvalue[k] <- 1 - (ptukey(DIF[k]/sdtdif, odif[k], 
                               DFerror))^(ntr/odif[k])
    if (odif[k] > ntr - 2) 
      pvalue[k] <- 1 - (ptukey(DIF[k]/sdtdif, odif[k], 
                               DFerror))
  ...
  parameters <- data.frame(test = "REGWQ", name.t = name.t, 
                           ntr = ntr, alpha = alpha)
  rownames(parameters) <- " "
  rownames(statistics) <- " "
  ...
  output <- list(statistics = statistics, parameters = parameters, 
                 regw = regw, means = means, comparison = comparison, 
                 groups = groups)
  class(output) <- "group"
  invisible(output)
}
